...
export function getContributorBySlug(file, detail = false) {
  ...
  return {
    ...
    weekPoints: getLastWeekPoints(calendarData),
...
export function getLastUpdated() {
  const lastUpdated = fs.readFileSync( join(process.cwd(), "data/last_updated"), "utf8");
  return lastUpdated;
}
...
const getLastWeekPoints = (calendarData) => {
  const lastWeek = calendarData.slice(-7);
  ...
  return  `${highlights.eod_update} eod_update * ${(points.eod_update ?? 0)}p = ${highlights.eod_update * (points.eod_update ?? 0)}p\n` +
          `${highlights.comment_created} comment_created * ${(points.comment_created ?? 0)}p = ${highlights.comment_created * (points.comment_created ?? 0)}p\n` +
          `${highlights.pr_opened} pr_opened * ${(points.pr_opened ?? 0)}p = ${highlights.pr_opened * (points.pr_opened ?? 0)}p\n` +
          `${highlights.pr_reviewed} pr_reviewed * ${(points.pr_reviewed ?? 0)}p = ${highlights.pr_reviewed * (points.pr_reviewed ?? 0)}p\n` +
          `${highlights.pr_merged} pr_merged * ${(points.pr_merged ?? 0)} = ${highlights.pr_merged * (points.pr_merged ?? 0)}\n` +
          `${highlights.pr_collaborated} pr_collaborated * ${(points.pr_collaborated ?? 0)}p = ${highlights.pr_collaborated * (points.pr_collaborated ?? 0)}p\n` +
          `${highlights.issue_assigned} issue_assigned * ${(points.issue_assigned ?? 0)}p = ${highlights.issue_assigned * (points.issue_assigned ?? 0)}p\n` +
          `${highlights.issue_opened} issue_opened * ${(points.issue_opened ?? 0)}p = ${highlights.issue_opened * (points.issue_opened ?? 0)}p\n` +
          `Bonus EOD: ${highlights.eod_update == 7 ? 20 : highlights.eod_update == 6 ? 10 : 0}p\n` + 
          `Total ${highlights.points} points`;
};
